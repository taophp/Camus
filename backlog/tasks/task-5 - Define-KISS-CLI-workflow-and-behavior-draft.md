...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A documented command reference exists (this ticket), describing flags, behavior, and exit codes.
- [ ] #2 `kiss build --dev` semantics are unambiguously defined and documented.
- [ ] #3 `kiss test` behavior is specified: it must persist seeds, test hashes, logs and provide a machine-readable summary useful for CI.
- [ ] #4 `kiss certify` flow is fully described, including when it triggers tests and how it records metadata in `kiss.lock`.
- [ ] #5 A first-pass `kiss.lock` schema (YAML/JSON) is included and reviewed in the repo.
- [ ] #6 At least two CLI PoCs are planned (formatter, basic test runner) with concrete subtasks and acceptance tests.
- [ ] #7 CI gating rules are defined (e.g., `kiss test` required, `kiss certify` required before publish).
- [ ] #8 Open questions (signing UX, key management, sandboxing external tests) are enumerated and assigned to follow-up tickets
<!-- AC:END -->
