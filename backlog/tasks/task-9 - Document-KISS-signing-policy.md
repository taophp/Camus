...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Draft `SIGNING_POLICY.md` (or a dedicated section in `60-design.md`) is added describing the source-only signing policy and preconditions.
- [ ] #2 `spec/kiss.lock.schema.yaml` includes the signature metadata fields (example schema added).
- [ ] #3 `kiss certify` and `kiss verify` UX is documented (interactive signing by default).
- [ ] #4 A small verification script or prototype (e.g., `scripts/ci-verify-kiss-lock.*`) is added as a PoC to check `kiss.lock` signature fields.
- [ ] #5 CI policy ticket references signing verification and a sample release workflow that rejects unsigned/dev artifacts.
- [ ] #6 A follow-up implementation ticket exists to add:
  - `kiss certify` sign stub / `kiss sign` UX,
  - `kiss verify` validation stub,
  - unit tests that assert cascade invalidation behavior.
- [ ] #7 Security follow-up ticket created to design key management & signing automation policy.
<!-- AC:END -->
