---
title: "001 — Formalize vocabulary: `claim`, `tests` vs `clues`"
status: "To Do"
labels: ["spec","design","vocab"]
assignee: ""
milestone: "Language specifications"
---

# Objective
Formalize and fix the terminology and semantics around the chosen concept (`claim`) and the associated proof mechanisms (`tests` and/or `clues`). The expected deliverable is a documented decision (in `60-design.md`) and a minimal set of examples and schemas that serve as a reference for implementations (`kiss`, spec, examples).

> Short context: the team chose `claim` to denote the declared behavior of a function/module. We still need to decide how proofs/requirements attach to a `claim`, the conventions (`tests` vs `clues`) and how `kiss` should consume and archive them.

---

# Initial proposal (to be validated in this ticket)
- Primary term for the "claim": `claim` (already accepted).
- Common name for executable evidence: `tests` — an **executable** field, required to certify a `claim`.
- Optional name for non-executable evidence or documentary items: `clues` — a **non-executable** field (links, rationale, audit references, formal invariants, human notes).
- Practical rule: `kiss test` executes the `tests`. `clues` are visible to the human auditor but are not executed automatically.
- `kiss certify`: if `tests` have not been executed/passed, `kiss certify` runs `kiss test` automatically. `kiss certify` records seeds, hashes, logs and results into the lock/artifact metadata.
...
- [ ] Decisions are referenced from ticket `000 — Initialize the backlog...` and linked from `60-design.md`.
