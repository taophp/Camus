...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `spec/test-result.schema.yaml` (or JSON Schema) is drafted and committed covering `meta`, `test_descriptor`, `result`, `hashes`, and `notes`.
- [ ] #2 `kiss.lock` schema updated to reference `test_run_id`, `tests_hash`, `test_seed`, and `test_artifact_path` (examples added to `spec/`).
- [ ] #3 Canonicalization rules (JCS or equivalent) and hash format are documented and a short HOWTO is available in the `spec/` directory.
- [ ] #4 Example test-run artifacts (for Hello and Inventory) are added under `Camus/examples/.kiss-test-examples/`.
- [ ] #5 A PoC ticket exists to implement `kiss test` that writes a test-run artifact and updates `kiss.lock`.
- [ ] #6 A PoC ticket exists for `kiss test --replay` to support deterministic re-runs and diagnostics.
- [ ] #7 CI integration guidance is documented (how to upload artifacts and validate test runs during release).
<!-- AC:END -->
