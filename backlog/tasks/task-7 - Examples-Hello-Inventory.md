...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `Camus/examples/hello.camus` exists and contains a `claim` and at least one example-based `test`.
- [ ] #2 `Camus/examples/inventory.camus` exists and contains:
 - a module-level `claim`,
  - at least one example-based `test`,
  - at least one property-based `test` with an explicit `seed`,
  - function-level `claim` + tests for `addItem` (and ideally `removeItem` / `listItems`).
- [ ] #3 `Camus/examples/README.md` documents:
  - how to run tests (intended `kiss test` commands or prototype scripts),
  - how to interpret recorded test metadata (seed, test hash, run logs).
- [ ] #4 `Camus/examples/kiss.lock.example` demonstrates a sample lockfile entry (source_hash, tests_hash, test_seed, test_result).
- [ ] #5 `Camus/SPEC.md` and `Camus/60-design.md` reference these example files.
- [ ] #6 Examples are reviewed by at least one project maintainer (review recorded on the backlog ticket or PR).
<!-- AC:END -->
